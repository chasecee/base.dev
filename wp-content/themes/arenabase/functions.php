 ...
 */
function container_wrap_before() {
	if( !is_page( array( '15' ) )) :
		echo "<div class='container'>";
	endif;
}
function container_wrap_after() {
	if( !is_page( array( '15' ) )) :
		echo "</div>";
	endif;
}
add_action('container_wrap_before',  'container_wrap_before');
add_action('container_wrap_after',  'container_wrap_after');
...
function my_password_protected_login_head() {
   ...
   ?>
   <style>
   .login h1 a {background-size: contain;width: auto;background-image: url('<?php echo $logo_url; ?>');}
   </style>
   <?php
}
